...
while IFS= read -r PACKAGE; do
    ...
    echo "  Attempting to fetch from Chainguard..."
...
EOF
    ...
    if [[ "$DEBUG" == "true" ]]; then
        echo "  DEBUG: npmrc contents:"
        cat "$CGR_NPMRC" | sed 's/_password=.*/_password=<REDACTED>/'
        echo "  DEBUG: Running npm pack command..."
    fi
    ...
    npm pack "$PACKAGE_NAME@$PACKAGE_VERSION" \
        ...
        > "$NPM_OUTPUT" 2>&1 || NPM_EXIT_CODE=$?

    if [[ "$DEBUG" == "true" ]]; then
        echo "  DEBUG: npm pack exit code: $NPM_EXIT_CODE"
        echo "  DEBUG: npm pack output:"
        cat "$NPM_OUTPUT"
    fi
    ...
    if [[ $NPM_EXIT_CODE -eq 0 ]]; then
        ...
        if [[ -n "$TARBALL" && -f "$TARBALL" ]]; then
            ...
            if [[ "$PACKAGE_NAME" == @*/* ]]; then
                ...
                ORIGIN_CMD=(aws codeartifact put-package-origin-configuration \
                    --domain "$DOMAIN_NAME" \
                    --repository "$REPO_NAME" \
                    --format npm \
            ...
            if ! "${ORIGIN_CMD[@]}" > /dev/null 2>&1; then
                if [[ "$DEBUG" == "true" ]]; then
                    echo "  DEBUG: Failed to set package origin config (may already be set)"
                fi
            fi
...
EOF
            ...
            if [[ "$DEBUG" == "true" ]]; then
                echo "  DEBUG: CodeArtifact npmrc contents:"
                cat "$CA_NPMRC" | sed 's/_authToken=.*/_authToken=<REDACTED>/'
                echo "  DEBUG: Tarball: $TARBALL"
            fi
            ...
            if [[ "$DEBUG" == "true" ]]; then
                echo "  DEBUG: npm publish exit code: $PUBLISH_EXIT_CODE"
                echo "  DEBUG: npm publish output:"
                cat "$PUBLISH_OUTPUT"
            fi
